...
public class PrimVsKruskal1{
	...
	*/
	static boolean PrimVsKruskal(double[][] G){
		...
		double weight_p, weight_k = 0;
		EdgeWeightedGraph Gt = new EdgeWeightedGraph(n);
		...
		PrimMST mst_p = new PrimMST(Gt);
		for (Edge e : mst_p.edges()) {
			System.out.println(e);
			str_PrimMST += e.toString();
			str_PrimMST += "\n";
		}
//		System.out.println(str_PrimMST);
		weight_p = mst_p.weight();
		System.out.println(weight_p);

		System.out.println("Kruskal Tree:");
		KruskalMST mst_k = new KruskalMST(Gt);
