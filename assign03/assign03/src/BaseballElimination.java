...
public class BaseballElimination{
	...
	*/
	...
	public BaseballElimination(Scanner s)
	{
		...
		//read the input file or information and seperate to team name, win , games remaining.
		...
		// first loop to make the flow network and eliminated arraylist.
//		use loop to read every team.
		for (int i = 0; i <size;i++) 
		{
//			 current team
			String current = AL.get(i);
			int pos = i;
//			if current team is eliminated, and it is out.
...
//			loop the team to 
			for (int j = 0; j < size; j++) 
			{
				if (j != pos ) 
				{
//					if the team win game and remaining game is smaller than the other team, take them out.
...
//				n point has n*(n-1)/2 lines
				...
				System.out.println(FNet.toString());// test
